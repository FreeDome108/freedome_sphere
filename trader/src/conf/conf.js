...
export const markets_configs = {
    ...
    [
        ...
        {
            exchange: "deribit",
            trading_pair: 'BTC-FS-27DEC24_PERP',
            future:
            {
                expire_date:new Date('2024-12-27')
            }
...
/*
    догружаем реальные данные при инициализации и кладем в конфиг
    const makerFee = makerExchange.markets[makerMarketId].taker; // Процент комиссии maker (предполагаем taker комиссию для худшего случая)
    const takerFee = takerExchange.markets[takerMarketId].maker; // Процент комиссии taker

    // Для простоты проскальзывание задаем как константу, можно адаптировать под реальные условия
    const makerSlippage = 0.05; // Пример 0.05% проскальзывания для maker
    const takerSlippage = 0.05; // Пример 0.05% проскальзывания для taker

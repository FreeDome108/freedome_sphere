...
class DManV1ScriptConfig(BaseClientModel):
    ...
    maker_pair: str = Field("XMR-USDT", client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Enter the trading pair for the maker market (e.g., XMR-USDT):"))

    # Taker exchange configuration
    taker_exchange: str = Field("binance_perpetual", client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Enter the exchange name where the bot will make arbitrage makert orders (e.g., binance_perpetual):"))
    taker_pair: str = Field("XMR-USDT", client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Enter the trading pair for the taker market (e.g., XMR-USDT):"))

    # Orders configuration
    order_amount: Decimal = Field(10, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Enter the base order amount in quote asset (e.g., 10 USDT):"))
    profitability_target: Decimal = Field(0.06, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "The target profitability for open limit order on maker market in percentage. (e.g., 0.6 for 0.6% gain):"))
    profitability_min: Decimal = Field(0.05, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "The maximum profitability for open limit order on maker market in percentage. (e.g., 0.7 for 0.7% gain):"))
    profitability_max: Decimal = Field(0.07, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "The minimum profitability for open limit order on maker market in percentage.(e.g., 0.5 for 0.5% gain):"))

...
Обратно совместим с форматом .comics

Дополнения, не влияющие на работу оригинала:
1. Z-depth (По умолчанию 100, range 0-108, float)
...
При импорте автоматически генерируется перевод с использованием локальной модели mozgach108/minimal в режиме квантовой запутанности (параллельный запуск 108 экземпляров в предзагруженных моделях для начала тоже подойдет) для перевода каждой фразы одновременно на 108 языков

В промпт также вводится тип баллона, отношение сторон баллона, коэффициент аналоговости/цифровости (где 100% - это прямоугольная форма, в других случаях - овальная)

Обратная совместимость работает так, что при попытке воспроизвести или редактировать старыми средствами - приложением Mahabharata и Comica Pro - действия возможны, но они работают с оригинальными картинками, а дополнения .boranko могут быть утеряны (не известно как работают legacy приложения)
